  ...
  return (
    <div className="rounded-lg border border-outline bg-surface p-5 md:p-6">
      <div className="flex items-baseline justify-between mb-6">
        ...
        <span className="text-xs uppercase tracking-[0.14em] text-on-surface-faint font-mono">{stages.length} etapas registradas</span>
      </div>
      ...
      <div className="flex flex-col gap-3">
            ...
            <div key={s.sort_order} className="flex items-center gap-4">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-baseline justify-between mb-1.5 gap-2">
                  <span className="text-[14px] text-on-surface font-medium truncate">{s.stage}</span>
                  <div className="flex items-baseline gap-3 shrink-0">
                    ...
                    <span className="text-[14px] font-mono text-primary tabular w-14 text-right">{s.leads.toLocaleString("es-CO")}</span>
                  </div>
                </div>
                <div className="h-2.5 rounded-full bg-background border border-outline overflow-hidden">
                  <div
                    className={`h-full rounded-full transition-[width] duration-500 ease-out ${isFirst ? "bg-primary" : s.ingresos > 0 ? "bg-secondary" : "bg-primary/70"}`}
        ...
        {stages.length === 0 && <p className="text-[14px] text-on-surface-variant py-4">Sin datos para esta campaña todavía.</p>}
      </div>
    </div>
  );
}
