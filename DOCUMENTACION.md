...
N8N_CAPTACION_LEAD_URL=.../webhook/integraciones/captacion-lead
NEXT_PUBLIC_CAPTACION_LEAD_URL=.../webhook/integraciones/captacion-lead
```

**Las seis ya están replicadas en Vercel** (Production/Preview/Development) — a diferencia de gotchas anteriores de este mismo tipo (secciones 16/17), esta vez se agregaron vía API de Vercel en la misma sesión en que se crearon, antes del deploy. `NEXT_PUBLIC_CAPTACION_LEAD_URL` es la única pública a propósito: es el mismo valor de `N8N_CAPTACION_LEAD_URL`, pero el cliente la ve tal cual en el panel "Puntos de captación" para armar el enlace que pega en su landing — no expone ningún secreto, es literalmente la URL que se le entrega.

### Pendientes / deferred explícitamente

- **OAuth de Meta** — la conexión V1 es 100% manual (pegar ID + token); OAuth quedó como idea a futuro, no en el roadmap inmediato.
- **Prueba en vivo de pausar/activar** — ver gotcha arriba, bloqueada por el clasificador de seguridad de este asistente, pendiente de que el equipo la confirme manualmente.
...
- **Sin datos diarios/tendencia** — Análisis de Anuncios no tiene una columna de sparkline (como sí tiene el dashboard de referencia que mostró el usuario) porque el pull actual solo trae totales de 30 días agregados; traer series diarias por anuncio multiplicaría las llamadas a Meta por cada fila del grid — se dejó fuera a propósito en vez de inventar una tendencia falsa.
...
- **Prueba en vivo de la rama GHL de captación** — el camino sin GHL conectado (guardar el lead) se probó de punta a punta con datos reales; la rama que busca/crea/etiqueta el contacto en GHL se probó solo estructuralmente (`validate_workflow` + revisión de la lógica), porque no hay una sub-cuenta de GHL de prueba disponible en esta sesión. Queda pendiente que alguien del equipo conecte una cuenta real de GHL a `cliente-prueba-draft` y dispare un lead de prueba para confirmar el flujo completo.
