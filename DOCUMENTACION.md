...
**Ninguna de las dos está replicada en Vercel todavía** (solo en `.env.local`) — mismo tipo de gotcha ya documentado en la sección 16 con `N8N_PAGINAS_URL`/`N8N_WORDPRESS_*`: funciona perfecto en local y va a fallar con `"... no está configurada"` en producción hasta que alguien las agregue a mano en Vercel → Settings → Environment Variables y redeploye.
