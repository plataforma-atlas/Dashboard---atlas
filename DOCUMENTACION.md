...
- **Wizard** de 6 pasos, navegables libremente por pestañas numeradas (no es un flujo lineal forzado): (1) nombre/slug/tipo de funil, (2) 3 colores (`primario`/`fondo`/`texto`, con `<input type="color">` + hex a mano), (3) copy de captura (título, subtítulo, bullets dinámicos, texto del botón, checkbox "pedir WhatsApp", imagen opcional), (4) encuesta (preguntas dinámicas, cada una `abierta` u `opciones` con las opciones separadas por coma), (5) copy de gracias (título, subtítulo, botón + link, imagen opcional), (6) revisión.
- El paso de **revisión** genera la vista previa llamando a `/api/paginas/generar-html` (con `pagina_id: "preview"` si todavía no se guardó) y la muestra en un `<iframe sandbox>` con pestañas Captura/Encuesta/Gracias — HTML real, no una maqueta. Desde ahí: "Guardar borrador" (solo persiste, `POST /api/paginas`) o "Guardar y publicar en WordPress" (guarda, genera el HTML con la `site_url` real de la conexión activa, y llama a `/api/paginas/publicar-wordpress`, mostrando el link publicado o el error de cada una de las 3 páginas). Si el cliente no conectó WordPress, ese botón se reemplaza por un atajo a Configuración.
- v1 **no genera copy con IA** — todo el texto lo escribe el cliente/agencia a mano en el wizard, tal como se definió al arrancar esta pieza. Tampoco hay todavía más de una plantilla visual (`plantilla: "clasica-01"`, hardcodeada) ni edición de imágenes (son URLs pegadas a mano, sin upload propio).

Probado en local de punta a punta con la cuenta de prueba de la sección 20/24 (crear página → bullets/preguntas dinámicas → vista previa de las 3 páginas → guardar → volver a la lista → editar y confirmar que los datos vuelven tal cual se guardaron). Falta probar el botón de publicar contra un WordPress real (en local la cuenta de prueba no tiene WordPress conectado); el mecanismo de publicación en sí ya se probó por separado (ver arriba, prueba contra `wordpress.org`).
