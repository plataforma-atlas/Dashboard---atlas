...
- **Wizard** de 9 pasos, navegables libremente por pestañas numeradas (no es un flujo lineal forzado, salvo el paso "Encuesta" que solo aparece si el flujo elegido lo incluye — ver más abajo). Rediseñado en esta misma sesión para explicar cada elección como lo hace la referencia externa (Zoryam) en vez de ser un simple formulario:
  ...
  4. **Modelo** — selector de plantilla visual con las secciones que incluye cada una (hoy solo "Clásica" está disponible; "Urgencia" y "Webinario + VSL" están listadas como "Próximamente", mismo patrón visual que "Whop" en Conexiones), más el checkbox "pedir WhatsApp" y el selector de formato del campo WhatsApp del popup (`internacional` — código de país + número, default; o `brasil` — DDD + número), que cambia el placeholder del input en el HTML generado.
  5. **Colores** — 3 colores (`primario`/`fondo`/`texto`, con `<input type="color">` + hex a mano) — sin cambios.
  6. **Copy — Captura** — título, subtítulo, bullets dinámicos, texto del botón, imagen opcional (el checkbox de WhatsApp se movió al paso "Modelo").
...
- v1 **no genera copy con IA** — todo el texto lo escribe el cliente/agencia a mano en el wizard, tal como se definió al arrancar esta pieza. Tampoco hay todavía más de una plantilla visual real (`plantilla: "clasica-01"`) ni edición de imágenes (son URLs pegadas a mano, sin upload propio).
...
**Pendiente (explícitamente fuera de esta iteración, a pedido del usuario — "vamos hasta acá y ya vamos con lo demás")**: el paso "Copy" todavía es un formulario plano sin la riqueza explicativa de "Funil"/"Flujo"/"Modelo"; no hay plantillas visuales reales más allá de "Clásica"; no hay upload de imágenes propio (solo URLs pegadas a mano); no hay un paso "Cliente" explícito en el wizard (no hace falta — el dashboard ya resuelve el cliente activo por sesión/URL antes de entrar al wizard, a diferencia de la referencia externa que es multi-tenant desde cero en cada pantalla).
