...
Primera pieza de la visión de "self-service": el cliente arma su propio embudo y conecta sus propias cuentas de Meta/GHL desde el dashboard, sin que nadie del equipo tenga que tocar n8n a mano por cada cliente nuevo. Fuente: `app/panel/embudos/page.tsx`, `app/api/embudo/configurar/route.ts`, `app/api/onboarding/conectar-meta-ads/route.ts`, y en n8n los workflows `Núcleo — Embudo Webinar (Captación)`, `Núcleo — Leads y Compradores`, `Núcleo — Configurar Embudo`, `Integraciones — Pull Meta Ads (Cliente)` e `Integraciones — Pull GoHighLevel (Cliente)` (carpetas "07 · Núcleo Compartido" y "11 · Integraciones Cliente" en n8n).
...
**"Configuración" y "Embudos" abren como panel flotante, no navegan**: en los tres sidebars (dashboard clásico, Control Center, `AppSidebar`), estos dos items son `<button>` que abren un panel deslizante encima de la pantalla actual (`components/SidePanelProvider.tsx`, contexto `useSidePanel()` provisto en `app/layout.tsx`) — el usuario nunca pierde el dashboard que estaba viendo. El contenido real vive en `components/panel/ConexionesBody.tsx` y `components/panel/EmbudosBody.tsx`, reusado tanto por el panel flotante como por las páginas standalone `/panel/conexiones` y `/panel/embudos` (que siguen existiendo para acceso directo por URL, con su propio `AppSidebar` + layout). De paso se quitó el link "Dashboard clásico" del sidebar del Control Center — ya no tenía una función clara una vez agregados estos dos accesos.
