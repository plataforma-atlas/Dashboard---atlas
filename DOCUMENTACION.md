...
**Las cuatro ya están replicadas en Vercel** (Production/Preview/Development) — se agregaron vía API de Vercel en la misma sesión en que se crearon, antes del deploy. `N8N_CAPTACION_LEAD_URL` y `NEXT_PUBLIC_CAPTACION_LEAD_URL`, que sí llegaron a existir brevemente, se **eliminaron** (ver "Endpoints enmascarados" arriba) — los endpoints públicos del embudo ya no usan variables de entorno, van todos por `/api/hooks/[...path]` + `N8N_WEBHOOK_BASE_URL` (que ya existía desde la sección 20).
