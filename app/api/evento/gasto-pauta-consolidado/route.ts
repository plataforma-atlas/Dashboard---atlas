...
export async function GET(req: Request) {
  ...
  const url = process.env.N8N_EVENTO_AD_SPEND_CONSOLIDADO_URL;
  ...
  try {
    ...
    return NextResponse.json({ adSpendConsolidated: data });
  } catch (err) {
    console.error("Error consultando gasto consolidado:", err);
