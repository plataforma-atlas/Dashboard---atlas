...
export async function POST(req: Request) {
  ...
  const url = process.env.N8N_EVENTO_QUITAR_VIP_URL;
  if (!url) return NextResponse.json({ error: "N8N_EVENTO_QUITAR_VIP_URL no está configurada" }, { status: 500 });
  ...
  try {
    ...
    if (!res.ok) return NextResponse.json({ error: data.error || "No se pudo quitar el VIP" }, { status: res.status });
    return NextResponse.json(data);
  } catch (err) {
    console.error("Error quitando el VIP:", err);
